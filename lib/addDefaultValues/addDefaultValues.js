import { unregisterSchema, validate } from "@hyperjump/json-schema/draft-2020-12";
import { ParseAndValidateSchema, resolveRef } from "../helpers.js";
...
export class JsonSchemaProcessor {
  ...
  addDefaultsToinstance = async () => {
    try {
      const { schema: validatedSchema, result } = await ParseAndValidateSchema.validate(
        this.schema,
        this.instance
      );
      if (result.valid) {
      ...
      // } catch (error) {
      //   throw Error("Error parsing schema", { cause: error });
    } finally {
      unregisterSchema(this.schema.$id);
    }
  };
}
...
class SchemaDefaults {
  ...
  addDefaults = (schema, instance) => {
    if (instance === undefined && schema.default) {
      ...
      if (schema.anyOf || schema.oneOf) {
        ...
        return this.keywords._anyOf(arr, instance);
      }
      if (schema.if) {
        return this.keywords._if(schema, instance);
      }
...
class Keywords {
  _properties = (schema, properties, obj) => {
    ...
    // const handleObject = (propertyName, propertyValue, obj) => {
    //   if (propertyValue.type === "object" || obj.hasOwnProperty(propertyName)) {
    ...
    for (const propertyName in properties) {
      // console.log(111, properties, obj);
      const propertyValue = properties[propertyName];
      if (obj.hasOwnProperty(propertyName)) {
        // console.log(22222, propertyName);
        // handleRef(propertyName, propertyValue);
        // handleObject(propertyName, propertyValue, obj);
        new SchemaDefaults().addDefaults(propertyValue, obj[propertyName]);
  ...
  _patternProperties = (patternProps, obj) => {}

  _if = async (schema, obj) => {
    const schemaDefaults = new SchemaDefaults();
