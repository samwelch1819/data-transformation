...
export class JsonSchemaProcessor {
  ...
  addDefaultsToInstance = async () => {
    ...
    const schemaId = sortedSchema.$id;
...
const  sortDefaultsFirst = (schema) => {
  if (Array.isArray(schema)) {
    return schema.map(sortDefaultsFirst);
  } else if (schema !== null && typeof schema === "object") {
    // First, sort the keys with "default" first (if it exists)
    const sortedSchema = {};
    const keys = Object.keys(schema);

    if (keys.includes("default")) {
      sortedSchema["default"] = sortDefaultsFirst(schema["default"]);
    }

    for (const key of keys) {
      if (key === "default") continue; // Already handled
      sortedSchema[key] = sortDefaultsFirst(schema[key]);
    }

    return sortedSchema;
  }

  // Primitive values (string, number, boolean, etc) — return as is
  return schema;
}
...
const keywordHandlers = {
  ...
  "https://json-schema.org/keyword/items": (items, instance) => instance,
